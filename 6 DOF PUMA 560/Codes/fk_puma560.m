function forward_kinematics_puma560(angles_deg)
% Set joint angles (degrees)
angles_deg = [0 45 56 87 34 165];

% Assign to workspace for Simulink
theta1 = deg2rad(angles_deg(1));
theta2 = deg2rad(angles_deg(2));
theta3 = deg2rad(angles_deg(3));
theta4 = deg2rad(angles_deg(4));
theta5 = deg2rad(angles_deg(5));
theta6 = deg2rad(angles_deg(6));
assignin('base','theta1',theta1);
assignin('base','theta2',theta2);
assignin('base','theta3',theta3);
assignin('base','theta4',theta4);
assignin('base','theta5',theta5);
assignin('base','theta6',theta6);
 %% Clear and initialize
    ...
    angles_deg = input('Angles: ');  % e.g., [0, 0, 0, 0, 0, 0]
    if numel(angles_deg) ~= 6
        error('Please enter 6 joint angles.');
    end
    target_theta = angles_deg * pi/180;  % Convert to radians
...
% Run MATLAB FK
forward_kinematics_puma560(angles_deg);
